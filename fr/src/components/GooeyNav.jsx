...
// Gooey, bubbly nav used across the app for primary navigation links.
const GooeyNav = ({
  items,
  animationTime = 600,
  particleCount = 15,
  ...
  const active = controlledActiveIndex ?? activeIndex;

  // subtle randomness for particle motion
      ...
      element.classList.remove("active");
        ...
        setTimeout(() => {
          try {
            ...
            // element could be gone during unmount; ignore
          }
        }, t);
    }
  };
  ...
  const triggerSelection = (liEl, index) => {
    if (!liEl) return;
  ...
  useEffect(() => {
    ...
    const activeLi = navRef.current.querySelectorAll("li")[active];
    if (activeLi) {
      updateEffectPosition(activeLi);
      textRef.current?.classList.add("active");
      if (!initializedRef.current && filterRef.current) {
  ...
  return (
    <>
      ...
      <div className="relative" ref={containerRef} style={colorVars}>
