...
const JobSeekerLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await jobSeekerAPI.login(formData);
  ...
  return (
    <div className="min-h-screen px-4 py-10 flex items-center justify-center">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/10 backdrop-blur-xl shadow-2xl p-6 sm:p-7 text-white">
        <div className="mb-6">
          <h1 className="text-2xl font-semibold tracking-tight">Job Seeker Login</h1>
          <p className="mt-1 text-sm text-white/70">Welcome back! Find your next role.</p>
        </div>
        ...
        <div className="mt-6 space-y-2 text-sm text-white/70">
          <p>
            ...
            <Link className="text-white underline decoration-white/30 underline-offset-4 hover:decoration-white/70" to="/jobseeker/signup">
              Sign up
            </Link>
          </p>
          <p>
